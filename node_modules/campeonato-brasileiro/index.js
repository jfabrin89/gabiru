    ...
    request(options, function(error, response, html) {
      if(!error) {
        ...
        $('.rodada').each(function() {
          ...
          // console.log(item.attr('data-round'));

          if(item.attr('data-round')) lista.push(rodada);
          // console.log(item.html());
          // console.log('##################################');

          // produto.rodada = item.attr('data-round');
          // var dateS = item.find('.game-date').text().split(' - ');
          // if(dateS.length > 0) {
          //   produto.data = dateS[1];
          //   produto.horario = dateS[2];
          //   produto.dia = dateS[0];
          // } else {
          //   produto.data = '';
          //   produto.horario = '';
          //   produto.dia = '';
          // }
          // produto.mandante = item.find('.game-club--principal').attr('title');
          // produto.visitante  = item.find('.game-club--visitor').attr('title');
          // produto.mandantePlacar = item.find('div.game-scoreboard-values').children().first().text();
          // produto.visitantePlacar = item.find('div.game-scoreboard-values').children().last().text();
          // produto.estadio = item.find('.game-location').text();
          // produto.cidade = item.find('.local').text();
          // produto.transmisao = [];
          // item.find('.tv').children('i').each(function(i) {
          //   var i = $(this);
          //   switch (i.attr('class')) {
          //     case 'icon-tv icon-tv-aberta':
          //     produto.transmisao.push('Tv Aberta');
          //     break;
          //     case 'icon-tv icon-tv-sportv':
          //     produto.transmisao.push('SportTV');
          //     break;
          //     case 'icon-tv icon-tv-pfc':
          //     produto.transmisao.push('PFC');
          //     break;
          //     case 'icon-tv icon-tv-indefinida':
